...
export function register(server: Server): void {
  server.tool(
    ...
    "list projects with optional folder and status filters.",
    {
      ...
      status: z.enum(["active", "on_hold", "completed", "dropped", "all"]).default("active"),
      limit: z.number().int().min(1).default(100),
    },
    async ({ folder, status, limit }) => {
      try {
        return textResult(await listProjectsData(folder, status, limit));
      } catch (error: unknown) {
        return errorResult(normalizeError(error));
      }
    }
  ...
  server.tool(
    "get_project",
    "get one project by id or name with root tasks included.",
    { project_id_or_name: z.string().min(1) },
    async ({ project_id_or_name }) => {
      try {
        return textResult(await getProjectData(project_id_or_name));
      } catch (error: unknown) {
        return errorResult(normalizeError(error));
      }
    }
  ...
  server.tool(
    "create_project",
    "create a project with optional folder, note, and sequential mode.",
    {
      name: z.string().min(1),
      folder: z.string().min(1).optional(),
    ...
    async ({ name, folder, note, sequential }) => {
      try {
        const projectName = escapeForJxa(name.trim());
        const folderName = folder === undefined ? "null" : escapeForJxa(folder.trim());
        const noteValue = note === undefined ? "null" : escapeForJxa(note);
...
const sequentialValue = ${sequentialValue};
const parent = (() => {
  if (folderName === null || folderName === "") return document;
  const folder = document.flattenedFolders.byName(folderName);
  if (!folder) throw new Error(\`Folder not found: \${folderName}\`);
  return folder;
})();
const project = new Project(projectName, parent.ending);
if (noteValue !== null) project.note = noteValue;
if (sequentialValue !== null) project.sequential = sequentialValue;
...
`.trim();
  ...
  server.tool(
    "complete_project",
    "mark a project complete by id or name.",
    { project_id_or_name: z.string().min(1) },
    async ({ project_id_or_name }) => {
      try {
        const projectFilter = escapeForJxa(project_id_or_name);
        const script = `
const projectFilter = ${projectFilter};
const project = document.flattenedProjects.find(item => {
  return item.id.primaryKey === projectFilter || item.name === projectFilter;
...
project.markComplete();
return { id: project.id.primaryKey, name: project.name, completed: project.completed };
`.trim();
        return textResult(await runOmniJs(script));
      } catch (error: unknown) {
        return errorResult(normalizeError(error));
...
export async function listProjectsData(
  ...
  status: "active" | "on_hold" | "completed" | "dropped" | "all",
  limit: number
): Promise<unknown> {
...
const projects = document.flattenedProjects
  .filter(project => {
    const normalizedStatus = project.status === Project.Status.Active ? "active"
...
export async function getProjectData(projectIdOrName: string): Promise<unknown> {
