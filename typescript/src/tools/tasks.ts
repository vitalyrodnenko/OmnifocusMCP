...
`.trim();
  ...
  server.tool(
    "move_tasks_batch",
    "move multiple tasks without deleting or recreating them. destination modes: (a) provide project to move tasks to a project, (b) provide parent_task_id to move tasks under an existing parent task, or (c) omit both to move tasks to inbox. runs as one omnijs call per invocation and returns per-task move results.",
    {
      task_ids: z.array(z.string().min(1)).min(1),
      project: z.string().min(1).optional(),
      parent_task_id: z.string().min(1).optional(),
...
`.trim();
  ...
  server.tool(
    "move_tasks_batch",
    "move multiple tasks in one omnijs call without deleting or recreating them. destination modes: (a) provide project to move to a project, (b) provide parent_task_id to move under an existing parent task, or (c) omit both to move to inbox.",
    {
      task_ids: z.array(z.string().min(1)).min(1),
      project: z.string().min(1).optional(),
      parent_task_id: z.string().min(1).optional(),
