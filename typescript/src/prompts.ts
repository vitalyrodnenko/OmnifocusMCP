...
import { getProjectData, listProjectsData } from "./tools/projects.js";
import { getInboxData, listTasksData } from "./tools/tasks.js";
    ...
    async () => {
      ...
      return {
        messages: [
          {
            role: "user",
            content: { type: "text", text: `daily review\n${JSON.stringify({ overdue, dueSoon, flagged })}` },
          },
        ],
      };
    }
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `weekly review\n${JSON.stringify({ activeProjects, availableTasks })}`,
            },
          },
        ],
      };
    ...
    async () => {
      ...
      return {
        messages: [
          {
            role: "user",
            content: { type: "text", text: `inbox processing\n${JSON.stringify(inboxItems)}` },
          },
        ],
      };
    }
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `project planning\n${JSON.stringify({ projectName, projectDetails, availableTasks })}`,
            },
          },
        ],
      };
